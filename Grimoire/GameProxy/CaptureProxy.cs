    ...
    public class CaptureProxy
    {
        ...
        private void _Listen()
        {
            ...
            while (!_exit)
            {
                try
                {
                    Console.WriteLine($"connect : {Destination.Address}:{Destination.Port}");
            ...
            while (!_exit)
            {
                ...
                for (int i = 0; i < read; i++)
                {
                    ...
                    else
                    {
                        ...
                        Interceptors.OrderBy(x => x.Priority).ForEach(x => x.Intercept(message, outbound));
                        if (message.Send)
                        {
